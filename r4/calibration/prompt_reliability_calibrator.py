...
class PromptReliabilityCalibrator:
    ...
    def __init__(
        ...
        self.prompt_q = torch.full((self.num_classes,), 0.50)
    ...
    def update_from_batch(
        ...
        new_prompt = self.prompt_q.clone()
        for c in range(self.num_classes):
            ...
            new_prompt[c] = torch.quantile(prompt_quality[:, c].reshape(-1), self.prompt_quantile)
        ...
        if self.fitted:
            ...
            self.prompt_q = keep * self.prompt_q + (1.0 - keep) * new_prompt
        else:
            self.teacher_q = new_teacher
            self.sam_q = new_sam
            self.agreement_q = new_agreement
            self.prompt_q = new_prompt
            self.fitted = True
        return self
    ...
    def gates(
        ...
        if sam_prob is not None:
            ...
            semantic_gate = semantic_gate & (sam_conf >= sam_thresh) & agree
            sam_train_gate = semantic_gate.clone()
            structure_gate = (sam_conf >= sam_thresh) & (teacher_conf >= teacher_thresh)
        if sam_iou is not None:
            iou_map = sam_iou.to(device).gather(1, teacher_arg.unsqueeze(1)).squeeze(1)
            sam_train_gate = sam_train_gate & (iou_map >= self.sam_q.to(device)[teacher_arg].clamp(max=0.95))
        if prompt_quality is not None:
            prompt_map = prompt_quality.to(device).gather(1, teacher_arg.unsqueeze(1)).squeeze(1)
            prompt_thresh = self.prompt_q.to(device)[teacher_arg]
            semantic_gate = semantic_gate & (prompt_map >= prompt_thresh)
            sam_train_gate = sam_train_gate & (prompt_map >= prompt_thresh)
        return {
    ...
    def load_state_dict(self, state):
        ...
        self.prompt_q = torch.tensor(state.get("prompt_q", [0.5] * self.num_classes)).float()
