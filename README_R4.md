...
- `r4/models/sam_peft.py`: SAM freezing plus real LoRA injection for selected image-encoder blocks.
...
Set `sam.checkpoint` in the YAML config. R4 carries a local SAM implementation copied from KnowSAM under `SAGE_SAM_R4/Model/sam`, so `segment_anything` is not required on the server. When `sam.use_sam=true`, R4 fails if the checkpoint is missing, the loaded SAM has no parameters, or `sam.train_peft=true` leaves SAM without injected LoRA or enabled Adapter parameters.
...
- Diagnostics include `total_sam_params`, `trainable_sam_params`, `trainable_sam_ratio`, `lora_param_count`, `adapter_param_count`, and trainable module names.
